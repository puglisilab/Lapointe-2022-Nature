function [mean_crosstalk_file, crosstalk_original] = calc_crosstalk_CA(input)
...
for i = 1:nTraces
    ...
    if numel(range) >= 0 %CARLOS CHANGED THIS FROM 8 to 0
        crosstalk(i) = mean(data.acceptor(i,range)) / mean(data.donor(i,range));
    end
end

% Saves the original crosstalk
crosstalk_original = crosstalk;
